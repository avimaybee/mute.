...
- **Context Awareness:** Reacts to real-time events like unlocking, charging, and low battery.
- **Remote Intelligence:** GitHub Gist synchronization for real-time prank updates without app store releases.
- **Reliability Protocol:** Periodic "Survival Heartbeat" to ensure app function even if system broadcasts are missed.
- **User Control:** "Master Toggle" to arm/disarm the system globally, plus persistent "System Logging" for transparency.
- **Hybrid Content:** Combines evergreen local pranks with remotely fetched daily content.
